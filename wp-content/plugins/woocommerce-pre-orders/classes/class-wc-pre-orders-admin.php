...
class WC_Pre_Orders_Admin {
	...
	public function __construct() {
		...
		add_action( 'woocommerce_process_product_meta_composite', array( $this, 'save_product_tab_options' ) );
		add_action( 'woocommerce_process_product_meta_bundle', array( $this, 'save_product_tab_options' ) );
	...
	public function load_styles_scripts( $hook_suffix ) {
		...
		if ( $this->page_id == $hook_suffix || 'edit.php' == $hook_suffix || 'post.php' == $hook_suffix || 'post-new.php' == $hook_suffix ) {
			...
			if ( ( $this->page_id == $hook_suffix ) && version_compare( WOOCOMMERCE_VERSION, '2.3.0', '>' ) ) {
	...
	*---------------------------------------------*/
	...
	public function add_product_tab() {
		...
		?><li class="wc_pre_orders_tab wc_pre_orders_options"><a href="#wc_pre_orders_data"><?php _e( 'Pre-Orders', 'wc-pre-orders' ); ?></a></li><?php
	}
