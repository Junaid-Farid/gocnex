...
public function woocommerce_cart_widget_css() {
	global $pagenow;
	if(is_admin()){
		wp_enqueue_style( 'woocommerce_cart_report_admin_gen_css', plugins_url() . '/woocommerce-cart-reports/assets/css/cart_reports_admin_general.css');
		if(($pagenow == 'index.php' )){
			if(function_exists('WC'))
	   			wp_enqueue_style( 'woocommerce_cart_report_admin_dashboard_css', plugins_url() . '/woocommerce-cart-reports/assets/css/cart_reports_admin_dashboard_mp6.css');
			else
	   			wp_enqueue_style( 'woocommerce_cart_report_admin_dashboard_css_legacy', plugins_url() . '/woocommerce-cart-reports/assets/css/cart_reports_admin_dashboard.css');
		}
	}
}
...
<ul class="wc_status_list">
	...
	<?php
		$num  = number_format_i18n( $nums_lifetime['Open'] );
		$text = __( 'carts currently open', 'woocommerce' );
		$link = add_query_arg( array( 'post_type' => 'carts', 'mv' => 'Open' ), get_admin_url( null, 'edit.php' ) );
	?>
	...
	<?php
		$num  = number_format_i18n( $nums_right_now['Abandoned']  ); //We're adding these together for the "Not Converted" field.
		$text =  __( sprintf("carts abandoned over the last %s Day%s", $days, $plural), 'woocommerce' );
		$link = add_query_arg( array( 'post_type' => 'carts', 'mv' => 'Abandoned','start_date' => date('Y-m-d', time() - (60*60*24 * $days)), 'end_date' => date('Y-m-d', time()) ), get_admin_url( null, 'edit.php' ) );
	?>
	<a href="<?php echo $link; ?>">
		<strong>
