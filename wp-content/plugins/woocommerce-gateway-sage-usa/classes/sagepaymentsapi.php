<?php
    ...
    class WC_Gateway_SagePaymentUSA_API extends WC_Payment_Gateway {
        ...
        public function __construct() {
			...
			$this->US_key           = $this->settings['US_key'];
			$this->US_id            = $this->settings['US_id'];
        ...
        function init_form_fields() {
			$this->form_fields = array(
				...
				'M_id'            => array(
				    'title'         => __( 'CAD Identification Number', 'woocommerce-spusa' ),
				    'type'          => 'text',
				    'description'   => __( '12 Digit Merchant Identification Number (Virtual Terminal ID). This should have been supplied by Sage when you created your account.', 'woocommerce-spusa' ),
				    'default'       => ''
				),
				'M_key'            => array(
				    'title'         => __( 'CAD  Merchant Key', 'woocommerce-spusa' ),
				    'type'          => 'text',
				    'description'   => __( '12 Digit Merchant Key Required for Gateway Access. This should have been supplied by Sage when you created your account.', 'woocommerce-spusa' ),
				    'default'       => ''
				),
				...
				'US_id'            => array(
				    'title'         => __( 'US Identification Number', 'woocommerce-spusa' ),
				    'type'          => 'text',
				    'description'   => __( '12 Digit Merchant Identification Number (Virtual Terminal ID). This should have been supplied by Sage when you created your account.', 'woocommerce-spusa' ),
				    'default'       => ''
				),
				'US_key'            => array(
				    'title'         => __( 'US Merchant Key', 'woocommerce-spusa' ),
				    'type'          => 'text',
				    'description'   => __( '12 Digit Merchant Key Required for Gateway Access. This should have been supplied by Sage when you created your account.', 'woocommerce-spusa' ),
				    'default'       => ''
				),
			...
			elseif ( ! empty( $this->cardtypes ) ) :
				...
				if ( get_option('woocommerce_force_ssl_checkout')=='no' ) :
					...
					foreach ( $this->cardtypes as $card_type ) {
						...
						$icon .= '<img src="' . 
									esc_url( $woocommerce->force_ssl( $this->get_plugin_url() ) . '/assets/card-' . 
									strtolower( str_replace(' ','-',$card_type) ) . '.png' ) . '" alt="' . 
									esc_attr( strtolower( $card_type ) ) . '" />';
					}	
				endif;
 ...
 global $woocommerce, $woocommerce_wpml, $sitepress;
     ...
     */
		...
		function process_payment( $order_id ) {
			...
			// make your query.
			if ( $this->status == 'live' ) :
				 if ($woocommerce->session->get('client_currency')=='CAD') :
				 	
				$data  = "m_id=" 		. 	$this->M_id; //use default canadian currency id and key
				$data .= "&m_key=" 		. 	$this->M_key;
				 else :
				 	$data  = "m_id=" 		. 	$this->US_id; //use US currency id and key
				$data .= "&m_key=" 		. 	$this->US_key;
				endif;
			else :
				
				$data  = "m_id=" 		. 	$this->M_id_testing; //use testing currency id and key
				$data .= "&m_key=" 		. 	$this->M_key_testing;			
			endif;
			$data .= "&T_ordernum=" 	. 	urlencode( $order_id );
			$data .= "&T_amt=" 			. 	urlencode( $order->order_total ); 
			
			// try adding currency to the payment plugin -- nathan jan 27 2015
			$data .= "&T_currency=" 	. 	urlencode( strtolower(get_woocommerce_currency()) ); 
			...
			if( is_wp_error( $res ) ) {
   				$woocommerce->add_error(__('Internal error', 'woocommerce-spusa') . '');
   				
   				if ( $this->debug == true ) {
   					$woocommerce->add_error(__('Debugging is on', 'woocommerce-spusa') . '');
   					$woocommerce->add_error(__('Error message : ', 'woocommerce-spusa') . '');

   					$error_string = $res->get_error_message();
   					$woocommerce->add_error( '<p>' . $error_string . '</p>' );
   				}
				...
				if ( $res['body'][1] == 'A' ) {
					 ...
					 * Successful payment
					 */
					$order->add_order_note( __('Payment completed', 'woocommerce-spusa') . '<br />(Approval Code: ' . substr($res['body'], 2, 6) . ')<br />Approval Msg: ' .substr($res['body'], 8, 32) );
					...
					$woocommerce->add_error(__('Payment error', 'woocommerce-spusa') . ': ' . substr($res['body'], 8, 32) . '');

					if ( $this->debug == true ) {
   						$woocommerce->add_error(__('Debugging is on', 'woocommerce-spusa') . '');

   						$woocommerce->add_error( '<p>' . $_POST[$this->id . '-card-number'] . '<br />' .
   													$_POST[$this->id . '-card-cvc'] . '<br />' .
   													$_POST[$this->id . '-card-expiry'] );

   						$woocommerce->add_error( print_r( str_replace('&','<br />',$data), TRUE ) );
   					}
